var SnailBait = function () {
   ...
   this.RUNNER_CELLS_WIDTH = 33; // pixels
   this.RUNNER_CELLS_HEIGHT = 39;
   ...
   this.runnerCellsRight = [
      { left: 1069, top: 473, 
        width: this.RUNNER_CELLS_WIDTH, height: this.RUNNER_CELLS_HEIGHT },
...
SnailBait.prototype = {
   ...
   equipRunnerForJumping: function () {
      var INITIAL_TRACK = 1,
          RUNNER_JUMP_HEIGHT = 240,
          RUNNER_JUMP_DURATION = 2000;
   ...
   createCoinSprites: function () {
      var BLUE_SPARKLE_DURATION = 100,
          BLUE_SPARKLE_INTERVAL = 500,
          GOLD_SPARKLE_DURATION = 500,
          GOLD_SPARKLE_INTERVAL = 100,
          coin;
      ...
      for (var i = 0; i < this.coinData.length; ++i) {
         if (i % 2 === 0) {
            coin = new Sprite('coin', 
                          ...
                          [ new CycleBehavior(GOLD_SPARKLE_DURATION,
                                              GOLD_SPARKLE_INTERVAL) ]);
         }
         else {
            coin = new Sprite('coin', 
                          new SpriteSheetArtist(this.spritesheet,
                          ...
                          [ new CycleBehavior(BLUE_SPARKLE_DURATION,
                                              BLUE_SPARKLE_INTERVAL) ]);
         }
   ...
   drawSprites: function() {
    var sprite;
	var RUNNER_ON_TRACK = false;
	var runner = this.runner;
	var platforms = this.platforms;
	
	for (var j=0; j < platforms.length; ++j) {
		var platform = platforms[j];
		var runnerLeft = runner.left + this.spriteOffset;
		var runnerRight = runnerLeft + runner.width;
		
		if (platform.left < runnerRight
				&& platform.left + platform.width > runnerLeft
				&& runner.track === platform.track) {
			RUNNER_ON_TRACK = true;
			break;
		}
	}

	console.log(RUNNER_ON_TRACK);
	if (RUNNER_ON_TRACK) {
		runner.runAnimationRate = this.RUN_ANIMATION_RATE;
	}
	else {
